...
<section class="main-container">
    <div class="centered-wrapper">
        ...
        if (isset($_GET['project_filter'])) {
            ...
            echo "<input id='filter' type='hidden' value='$project_id'>";

        } else {
            $sql = "SELECT * FROM timeGeneral WHERE submitted ='yes';";
            echo "<input id='filter' type='hidden' value='none'>";
        }
...
<script>
    ...
    $(document).ready(function readyDoc() {
        ...
        $( ".edit_entry" ).click( function() {
            ...
            $('#project_id').val($(this).attr("data-project"))
        });
                    ...
                    if (moment(date_for_edit+" "+end+" "+end_diem).isValid()) {
                        ...
                        if (moment(date_for_edit+" "+start+" "+start_diem).unix() < moment(date_for_edit+" "+end+" "+end_diem).unix()) {
                            ...
                            $.post('edit_entry_for_employees.php', {date:date_for_edit,start:start,end:end,start_diem:start_diem,end_diem:end_diem,project_id:project_id,description:description,time_id:time_id}, function() {
                                $('#list').load('load_entries_managetime.php',{filter:filter})
                            });
...
<div id="edit_entry_modal" class="modal" style='display:none;'>
    ...
    <?php
        if ($employee_allow_edit == 'yes') {
            echo "<div style='width:696px; margin: 0 auto; height:310px; background-color:#fff; margin-top:10%;'>
                ...
                <div style='height:20px;'>
                    ...
                    <select id='start_diem_edit' name='start_diem_edit' form='new_entry' style='padding: 0 0 0 4px; float:left; margin-left:5px; margin-top:-15px; height:34px; width: 40px;'>
                        <option>AM</option>
                        <option>PM</option>
                    </select>
            ...
            echo "<div style='width:696px; margin: 0 auto; height:240px; background-color:#fff; margin-top:10%;'>";
        }
    ?>
        ...
        <div style='height:20px; margin-left:20px;'>
            <select id='project_id' style='float:left;padding: 0 0 0 4px; margin-top:-15px; height:34px; width: 148px;'>
                <?php 
        ...
        <?php 
            if ($employee_allow_edit == 'yes') {
                echo "
                <button type='sumbit' id='save_edit' name='project_submit' style='float:right; margin-right:20px;width: 100px;
                    margin-top:-15px;
                    height: 34px;
                    border: none;
                    background-color: rgb(66, 85, 252);
                ...
                echo "
                <button type='sumbit' id='save_edit2' style='float:right; margin-right:20px;width: 100px;
                    margin-top:-15px;
                    height: 34px;
                    border: none;
                    background-color: rgb(66, 85, 252);
